...
# Applies the formula to get K, given M and N
def get_K(p):
...
if __name__ == "__main__":
    ...
    # The dataset is now in the final form: [(rating1, 'title0001'), (rating2, 'title0002'), ...]
    final_dataset = rdd.map(lambda x: (round_vote(x[1]), 1))

    # Counts the number of titles for each rating in the dataset
    vote_counts = final_dataset.reduceByKey(lambda x, y: x + y).sortByKey()
    #vote_counts.saveAsTextFile("counts.txt")
    ...
    second_dataset = rdd.map(lambda x: (round_vote(x[1]), x[0]))

    # Puts the dataset in the form of [(key1, hash_list1[]), (key2, hash_list2[]), ...]
    hashed_dataset = second_dataset.map(lambda x: (x[0], calc_hash(x[0], x[1]))).sample(withReplacement=False, fraction=0.01)

    bf_rdd = hashed_dataset.reduceByKey(lambda x,y: x + y)

    bloomfilters = bf_rdd.map(lambda x: (x[0], create_bloom_filter(x[0], x[1])))

    to_save = bloomfilters.sortByKey().map(
        lambda x: (x[0], broadcast_k.value, broadcast_m_values.value[x[0] - 1], x[1].tobytes())).collect()
